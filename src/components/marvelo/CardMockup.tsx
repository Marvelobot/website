...
export function CardMockup({
  ...
  return (
    <div
      className={`relative aspect-[3/4.2] w-full overflow-hidden rounded-2xl border border-white/10 ${className}`}
      style={{
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div
          className="absolute -inset-y-10 left-0 w-1/3 animate-sheen"
          style={{
            background:
      ...
      {/* Top tag */}
      <div className="absolute left-3 right-3 top-3 flex items-center justify-between text-[10px] uppercase tracking-[0.2em] text-white/70">
        <span className="rounded-full border border-white/10 bg-black/40 px-2 py-1 backdrop-blur">
          {role}
        </span>
        <span className="font-display text-[10px] text-cosmic-gold">PWR {power.toLocaleString()}</span>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        <div className="rounded-lg border border-white/10 bg-black/55 px-3 py-2 backdrop-blur">
          <div className="font-display text-sm tracking-widest text-white">{name}</div>
